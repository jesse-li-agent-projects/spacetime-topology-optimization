...
- dummy_plan.md
    - Not actually a real plan, just an example to show syntax of this index: list the
      ...
      leftover.
- fixed_geometry_sequence_optimization.md
    - Adds `seqopt`: optimize only the print-time field for a fixed geometry loaded from
      an .npz, minimizing the overheating proxy under layer-uniformity and continuity
      constraints. Also renames optimize.py/cli.py to stto.py/stto_cli.py.
