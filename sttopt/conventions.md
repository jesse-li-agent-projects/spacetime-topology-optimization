...
`tests/fixtures/*.npz` are golden-regression fixtures: frozen snapshots of what
`sttopt` itself produces, generated by `tests/fixtures/generate_fixtures.py` and
loaded via `conftest.load_fixture_npz`. They catch an unintended future change to that
output; they are not an independent correctness check (that role belongs to
`tests/matlab_reference.py`/`matlab_reference_loop.py`, transliterations of the
original MATLAB source kept deliberately in its own 1-indexed, F-order convention, and
swept against `sttopt` in `test_reference_sweep.py`). Fixture arrays are plain numpy
arrays, 0-indexed and C-order throughout, matching `sttopt`'s own conventions exactly
— no relabeling needed to use one.
...
`scipy.sparse.linalg.spsolve`) accumulate small numerical differences downstream of a
linear solve. Compare per quantity, not per test:
