...
def apply_density_filter(
    field: Float[Tensor, "nely nelx"], H: Tensor, Hs: Float[Tensor, " nel"]
) -> Float[Tensor, "nely nelx"]:
    """Apply the density filter to a field: `H @ field / Hs`, shape-preserving.

    Goes through `torch_util.symmetric_matmul` rather than a plain `H @ ...` so the
    filter stays inside the autograd graph even for outputs whose rows are
    differentiated with `is_grads_batched=True`.

    :param field: raw (unfiltered) field, `(nely, nelx)`.
    :param H: density filter, as built by `density_filter` and converted with
        `torch_util.symmetric_csr_to_tensor`.
