...
def _sensitivity_rows(
    ...
    Every step of the chain is autograd's, the density filter included; its sparse
    matmul survives a batched row only because `filters.apply_density_filter` states
    `H`'s symmetry (see `torch_util.symmetric_matmul`).

    The `k == 1` versus `k > 1` split and the one-hot batched-grad trick are shared with
    `seqopt` via `sensitivity.jacobian_rows`, whose `is_grads_batched` vmap has no
    batching rule for `FemSolve`'s backward, nor for a plain sparse matmul's (the
    continuity filter's `L @ ...`, which is not symmetric). A `k > 1` output must avoid
    both in its own graph -- true of every current `k > 1` row (start point, stage
    bounds) but a restriction on future ones, not a guarantee.
