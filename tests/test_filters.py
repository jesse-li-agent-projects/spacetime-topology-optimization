...
def test_symmetric_matmul_matches_dense_and_batches():
    """The whole point of `torch_util.symmetric_matmul` over a plain `H @ v`: it is
    differentiable inside a graph whose rows are taken with `is_grads_batched=True`,
    which a sparse CSR matmul is not (its backward transposes to CSC, which vmap cannot
    batch). Pin both the value and the batched gradient against a dense reference.
    """
    H, _ = density_filter(4, 3, 1.5)
    H_t = torch_util.symmetric_csr_to_tensor(H, "cpu", torch.float64)
    H_dense = H_t.to_dense()

    v = torch.rand(H_t.shape[0], dtype=torch.float64)
    seeds = torch.eye(2, dtype=torch.float64)

    def rows(matmul, matrix):
        leaf = v.clone().requires_grad_(True)
        y = matmul(matrix, leaf)
        outputs = torch.stack([y.sum(), (y**2).sum()])
        return (
            y,
            torch.autograd.grad(
                outputs, leaf, grad_outputs=seeds, is_grads_batched=True
            )[0],
        )

    y_ref, grad_ref = rows(torch.matmul, H_dense)
    y, grad = rows(torch_util.symmetric_matmul, H_t)

    torch.testing.assert_close(y, y_ref)
    torch.testing.assert_close(grad, grad_ref)

    with pytest.raises(RuntimeError, match="SparseCsc"):
        rows(torch.matmul, H_t)
